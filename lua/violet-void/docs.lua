...
function M.extras()
  local Extra = require("violet-void.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/violet-void/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/violet-void/config.lua", "\n(--@class violet-void%.Config.-\n})")
